...
const Connections = () => {
  ...
  if (loading) return <p className="text-center text-white my-10">Loading connections...</p>;
  if (!connections || connections.length === 0)
    return <h1 className="flex justify-center text-white my-10">No Connections Found</h1>;
    ...
    <div className="text-center my-10 px-4">
      ...
      {connections.map(({ _id, firstName, lastName, photoUrl, age, gender, about }) => (
        ...
        >
          ...
          <Link to={"/chat/" + _id}>
            <button className="btn btn-outline btn-secondary border-gray-600 text-gray-200 hover:bg-gray-700 transition">
              Chat
            </button>
          </Link>
        </div>
